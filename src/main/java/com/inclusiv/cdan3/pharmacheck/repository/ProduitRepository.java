...
public interface ProduitRepository extends JpaRepository<Produit, Long> {
    ...
    @Query(value = "SELECT produit.id, nom_commercial, classe_pharmaceutique, conditionnement, dci, nom_commercial, presentation, lien_image FROM public.produit INNER JOIN public.stock ON produit.id=stock.produitid AND stock.pharmacieid=:idPharmacie ORDER BY produit.id DESC LIMIT 4", nativeQuery = true)
